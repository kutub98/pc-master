...
function NavListMenu() {
  ...
  const renderItems = navListMenuItems.map(
    ({ icon, title, description, color }, key) => (
      <a href="#" key={key}>
        <MenuItem className="flex items-center gap-3 rounded-lg mx-auto ">
  ...
  return (
    <React.Fragment>
      ...
      >
        ...
        <MenuList className="hidden max-w-screen-xl mx-auto rounded-xl lg:block fixed 0  left-0 right-0 top-0 xl:left-0 xl:right-0 xl:top-0 xl:fixed container">
          <ul className="grid mx-auto grid-cols-4 gap-y-2">{renderItems}</ul>
        </MenuList>
      </Menu>
...
function NavList() {
  return (
    <div style={{ maxHeight: "600px", overflowY: "auto" }}>
      <List className="mt-4 mb-6 p-0 lg:mt-0 lg:mb-0 text-black lg:flex-row lg:p-1 mx-auto">
        <MobileItems />
        <NavListMenu />
        <LaptopItems />
      </List>
